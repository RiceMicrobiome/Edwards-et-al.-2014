...
my %opts = ();
getopts('b:m:u', \%opts); # b for barcode fastq and m for mapping file

my $usage = "Usage:
demultiplex.pl -b barcodes.fq -m mapping_file.txt

-b\tbarcode fastq file - a product of concat_fq.pl
-m\tQIIME mapping file with the sample names in the first column and barcodes in the Second
-u\tprint usage\n";
